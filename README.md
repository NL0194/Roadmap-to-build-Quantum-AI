# Roadmap-to-build-Quantum-AI

1. Learn How to Code Python
2. Learn How to Make Unique App
3. Learn How to Make Customized AI
4. Learn How to Code Quantum Hardware
5. Integrate Quantum Hardware with Customized AI


# 1. Learn How to Code Python
1.1. Learn Fundamentals of Python


      Type this in Google - Asabeneh/30-Days-Of-Python (Github)
      Link: https://github.com/Asabeneh/30-Days-Of-Python
      
      Type this in Google - Python Full Course for Beginners - Programming with Mosh (Youtube)
      Link: https://www.youtube.com/watch?v=K5KVEU3aaeQ


1.2. Learn Sentences of Python


      Type this in Google - Python Tutor
      Link: https://pythontutor.com/python-compiler.html#mode=edit


# 2. Learn How to Make Unique App
2.1. Build Small Project


      Type this in Google - Python-mini-projects
      Link: https://github.com/Python-World/python-mini-projects?tab=readme-ov-file

      
2.2.  Build Projects


      Type this in Google - Build Your Own X (Github)
      Link: https://github.com/codecrafters-io/build-your-own-x


# 3. Learn How to Make Customized AI
3.1. Type this in Google - Cloud Skill Boost


      Link: https://www.cloudskillsboost.google/


3.2. Go to Path - Beginner: Introduction to Generative AI Learning Path


      Link: https://www.cloudskillsboost.google/paths/118


3.3. Cloud Skill Boost Search Bar - Integrate Generative AI Into Your Data Workflow


      Link: https://www.cloudskillsboost.google/paths/1281


3.4. Cloud Skill Bost Search Bar - Deploy and Manage Generative AI Models


      Link: https://www.cloudskillsboost.google/paths/1283


3.5. Cloud Skill Boost Search bar - Build and Modernize Applications With Generative AI
      Link: https://www.cloudskillsboost.google/paths/1282
